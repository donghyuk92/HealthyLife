...
import android.example.com.midterm.PhotoDiary.util.ImageCache;
import android.example.com.midterm.PhotoDiary.util.ImageFetcher;
...
public class ImageGridFragment extends Fragment implements AdapterView.OnItemClickListener {
	...
	public ArrayList<String> getimages() {
		...
		ArrayList<File> arImageFile = new ArrayList<File>();
		
		ImageFileFilter imageFileFilter = new ImageFileFilter();
		ArrayList<String> images = new ArrayList<String>();
		String name;
		File yourDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM);

		for (File f : yourDir.listFiles()) {
			File yourfile = null;	
			if (f.isDirectory() && !f.isHidden()) {
				yourfile = new File(f, "");
				for (File ff : yourfile.listFiles()) {
					if (ff.isFile() && imageFileFilter.accept(ff) && ff.length() != 0) {
						arImageFile.add(ff);
					}
				}
			} else {
				if (f.isFile() && imageFileFilter.accept(f) && f.length() != 0) {
					arImageFile.add(f);
				}
			}
		...
		Collections.sort(arImageFile, new CustomComparator());
		
		for(int i = 0 ; i < arImageFile.size(); i++) {
			name = arImageFile.get(i).toString();
			images.add(name);
		}
		return images;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		try {
			final View v = inflater.inflate(R.layout.photo_gridview, container, false);
	...
	public void onDestroyView() {
		...
		Log.e("GridFragment", "OnDestroyView");
		mImageFetcher.flushCache();
		mImageFetcher.clearCache();
		mImageFetcher.closeCache();
	...
	public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
		...
		// i.putExtra("imgName", imagelist.get(position - 3));
		...
		// }
		// mImageFetcher.closeCache();
		SetupActivity.imageLink = imagelist.get((int) id);
		// getActivity().finish();
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public Object getItem(int position) {
			return position < mNumColumns ? null : imagelist.get(position - mNumColumns);
		}
		...
		public View getView(int position, View convertView, ViewGroup container) {
			...
			mImageFetcher.loadImage(imagelist.get(position - mNumColumns), imageView);
			return imageView;
		}
